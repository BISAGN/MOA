					...
					<form:form name="linkmaster" id="linkmaster" action="placement_company_reg_Action"
						...
						<div class="card-style mb-30">
							...
							<div class="row">
								...
								<div class="col-12 col-sm-12 col-md-6 col-lg-4">
									<div class="input-style-1">
										<label>Owner's Name<strong class="mandatory">* </strong></label> 
...
<!-- 								    </div> -->
								    ...
								    <div class="col-12 col-sm-12 col-md-6 col-lg-4">
									<div class="input-style-1">
										<label for="username">Website URL<span class="mandatory">*</span></label>
...
<!-- 								<li><input class="main-btn info-btn btn-hover" type="button" value="Search" id="search_reg" name="search_reg"  /></li> -->
								...
								<li><a href="Enterprise_HODReport_Url"
							...
							<li class="custom-d-none" id="rejbt"><input type="button"
								class="main-btn danger-btn btn-hover" value="Reject"  name="rejbt"  
								 >
							</li>
...
<!--                      	  <th><h6>Product And Services</h6></th> -->
          ...
          <div class="modal" id="ViewStudentsModel">
		<div class="modal-dialog modal-dialog-centered modal-lg">
			<div class="modal-content">

				<!-- Modal Header -->
				<div class="modal-header">
					<h4 class="modal-title">Intrested Students</h4>
					<button type="button" class="btn-close" data-bs-dismiss="modal"></button>
				</div>

				<!-- Modal body -->
				<div class="modal-body">
				  <input type="hidden" id="StudentsListToApprove" name="StudentsListToApprove">
				  <input type="hidden" id="StudentsEmailListToApprove" name="StudentsListToApprove">
				  
				<table class="table" id="ViewStudentsDataTBL">
					<thead>
						<tr>
							<th><h6>Ser No</h6></th> 
...
<!-- 							<th class='lastCol'><h6>Action</h6></th> -->
					...
					<tbody id="ViewStudentsDataTBLBody">
...
<!-- 					</tr> -->
				...
				<div class="modal-footer">
					...
					<ul class="buttons-group mainbtn">
						...
						<li id="appstudent" >
							<a class="main-btn info-btn btn-hover" data-bs-dismiss="modal" > Approve</a></li>
						<li><a class="main-btn dark-btn n btn-hover"
							type="button" data-bs-dismiss="modal">Close</a></li>
					</ul>
				</div>
			</div>
		</div>
...
// 		};
		...
		document.getElementById('appstudent').onclick = function() {
			return ApproveStudentData();
...
// 		var product = $("#product").val();
		...
		$.post("getFilter_Enterprise_HOD_data?" + key + "=" + value, {
			startPage : startPage,
			...
			address:address,state:state,per_district:per_district,pincode:pincode,hours_from:hours_from,hours_to:hours_to
			 ,web_url:web_url,photo_path:photo_path,photo_path_pic:photo_path_pic,status:status
			
		}, function(j) {
			$("#apprvchk").html("<h6>Select for </h6>");
			for (var i = 0; i < j.length; i++) {
				if (j[i].chekboxaction=="") {
					var checkbox = "<p></p>"
					jsondata.push([ j[i].ser,checkbox,
						j[i].company_name, 
						j[i].name,
		...
		$.post("getTotalEdu_Reg_Enterprise_HOD_dataCount?" + key + "=" + value, {
			company_name:company_name,
			name:name,
			email_id:email_id,
			mobile_no:mobile_no,
...
// 			};
	...
	document.querySelectorAll('.ADDSystem').forEach((items, index) => {
		items.addEventListener('click', event => {
			...
			var sid = document.getElementById('sid'+val).value;
...
// 	 var state = $("#CheckVal3").val();
	...
	if(a == ""){
		alert("Please Select the Enterprise for Approval"); 
	}
 
	else{
			$.post("Approve_enterprise_Data?"+key+"="+value, {a:a ,email_id :email_id,notified:notified }).done(function(j) {
			
				alert("Enterprise Approved.");
...
// 	 var state = $("#CheckVal3").val();
	...
	if(a == ""){
		alert("Please Select the Enterprise for Approval"); 
			...
			$.post("Reject_enterprise_Data?"+key+"="+value, {a:a}).done(function(j) {
			
				alert("Enterprise Rejected.");
...
function ViewData(id) {
	...
	$.post("GetIntrested_Students_Data?"+key+"="+value, {id:id}).done(function(j) {
		const StudentsList = [];
		const StudentsEmailList = [];
		$('#ViewStudentsDataTBLBody').empty();
				...
				if(j.length>0){
					...
					for (var i = 0; i < j.length; i++){
						 att= i+1;
						 if (j[i].status=='0') {
							 StudentsList.push(j[i].id);
							 StudentsEmailList.push(j[i].email);
							 j[i].status="Pending";
						}
						 else
							 j[i].status="Forwarded";
						 
						 
						$("table#ViewStudentsDataTBL").append('<tr align="center" id="tr_viewStudentData'+att+'"><td>'+att+'</td>'
					...
					$("#StudentsListToApprove").val(StudentsList);
					$("#StudentsEmailListToApprove").val(StudentsEmailList);
				}
				else{
				$("table#StudentsDataTBL").append('<tr align="center" id="tr_viewStudentData'+att+'">'
...
function ApproveStudentData(){
	...
	var studentsId = $("#StudentsListToApprove").val();
	var studentsEmails = $("#StudentsEmailListToApprove").val();
	
	$.post("ApproveIntrested_Students_Data?"+key+"="+value, {studentsId:studentsId, studentsEmails:studentsEmails}).done(function(j) {

				if(j.length>0){
					alert("Students Details Approved And Forwarded");
				}
				else{
					alert("Students Details Approved And Forwarded");
				}
...
function getDistrictper() {
	...
	$ .post( "getDistrictOnstate_placementhod_report?" + key + "=" + value, { selval : selval },
					function(j) {
