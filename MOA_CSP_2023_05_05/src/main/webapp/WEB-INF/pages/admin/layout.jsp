...
<head>
 ...
 <!-- Favicons -->	
  ...
  <!-- single select, search with select start -->
	...
	<link rel="stylesheet" href="assets/vendor/common_custom_responsive.css">
	<!-- common style end -->
...
//  	    } 
 	...
 	jQuery(document).ready(function() {
 		...
 		//csp of menu
 		document.querySelectorAll('.module_csp').forEach((items, index) => {
			items.addEventListener('click', event => {
				debugger;
				var val=parseInt(index);
				var cid= document.getElementById('module'+val).value;
				getmodule(cid);
			})
		});
 		document.querySelectorAll('.submodule_csp').forEach((items, index) => {
			items.addEventListener('click', event => {
				debugger;
				var val=parseInt(index);
				var cid = document.getElementById('submodule'+val).value;
				getsubmodule(cid);
			})
		});
 		document.querySelectorAll('.screen_csp').forEach((items, index) => {
			items.addEventListener('click', event => {
				debugger;
				var val=parseInt(index);
				var cid = document.getElementById('screen'+val).value;
				getpagelink(cid);
			})
		});
...
//    	    });
             ...
             // set current sub module
   			...
   			jQuery('a#Dp_scr'+localStorage.getItem("pagelink")).attr("class","screen_csp active");
			...
			<div class="col-md-6">
				<div class="d-flex align-items-center">
					<a href="#">
						...
						<span class="minist"><span class="hinditext">???? ????????</span><br>Ministry of<br> Ayush</span>
					</a>			   
