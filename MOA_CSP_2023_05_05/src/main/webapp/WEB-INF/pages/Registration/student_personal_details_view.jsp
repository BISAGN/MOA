				...
				<div class="col-lg-12 col-md-12 col-sm-12">
					<div class="card-style mb-30" id="card_view">
						<div class="tunnel_design">
							...
							<div class="square tunnel_visited">
								<a href="#" class="tunnel_text" id="tunnel_2" href="Edu_Det_Url">
									Basic Education Details</a>
							</div>
							<div class="square tunnel_visited">
					...
					<div class="card-style mb-30" id="card_view">
							...
							<div class="row">
								...
								<div class="col-lg-4 col-md-6 col-sm-6 col-12">
									<div class="custom-data-value">
										...
										<select hidden="hidden" name="pers_permanent_state"
											id="pers_permanent_state" class="form-control custom-d-none">
											<option value="0">-- Select State --</option>
											<c:forEach var="item" items="${getMedStateName}"
												varStatus="num">
												<option value="${item[0]}" name="${item[1]}">${item[1]}</option>
											</c:forEach>
										</select> <select hidden="hidden" name="district_id"
											id="pers_permanent_district" class="form-control custom-d-none">
											<option value="0">--Select--</option>
											<c:forEach var="item" items="${getMedDistrictName}"
												varStatus="num">
												<option value="${item[0]}" name="${item[1]}">${item[1]}</option>
								...
								<div class="col-lg-4 col-md-6 col-sm-6 col-12">
									<div class="custom-data-value">
										...
										<select hidden="hidden" name="pers_present_state"
											id="pers_present_state" class="form-control custom-d-none">
											<option value="0">-- Select State --</option>
											<c:forEach var="item" items="${getMedStateName}"
												varStatus="num">
												<option value="${item[0]}" name="${item[1]}">${item[1]}</option>
											</c:forEach>
										</select> <select hidden="hidden" name="pers_present_district"
											id="pers_present_district" class="form-control custom-d-none">
											<option value="0">--Select--</option>
											<c:forEach var="item" items="${getMedDistrictName}"
												varStatus="num">
												<option value="${item[0]}" name="${item[1]}">${item[1]}</option>
								...
								<div class="col-lg-4 col-md-6 col-sm-6 col-12">
									<div class="custom-data-value">
										<label>Corresponding Address </label> <span
											class="value-bind address" id="corre_address_label"></span> <select
											hidden="hidden" name="corre_state" id="corre_state"
											class="form-control custom-d-none">
											<option value="0">-- Select State --</option>
											<c:forEach var="item" items="${getMedStateName}"
												varStatus="num">
										...
										</select> <select hidden="hidden" name="corre_district"
						...
						<div class="field-box">
							<div class="row">
								...
								<div class="col-lg-12 col-md-12 col-sm-12 col-12" id="neettable">
										...
										<div class="input-style-form-check_block check-multi-list">
											<div class="form-check checkbox-style">
												...
												<input type="hidden" id="hiddenUpdate" name="hiddenUpdate"
													...
													understand that my admission is provisional and is subject
													to final approval and enrollment by the University. I also
													understand that my provisional admission is without
													prejudice to the directives/rules and
													regulations/orders/confirmation from the designated and
													competent authorities of the state/central government or
													the hon'ble court. </label> <label class="check-list"> I
													hereby agree to abide by the terms and conditions or the
													rules pertaining to admission as prescribed by the
													competent authorities and admit that they are binding upon
													me legally and legitimately.</label> <label class="check-list">
													I undertake to pay the fees fixed by the Competent
													Authority, University.</label> <label class="check-list">I
													undertake to see daily notices exhibited on the noticed
													board of the college, observe and maintain a strict
													discipline as the student and otherwise, in the college
													premises including hostel and campus.</label>
											</div>
										</div>
									</div>
								</div>
						...
						<!-- <div class="col-lg-12 col-md-12 col-sm-12 col-12">
										<div class="d-flex">
											<div class="mr-5">
												...
												<div class="col-lg-12 col-12">
													<p class="und_text">
														<i class="bi bi-chevron-right"></i>I hereby agree to abide
														by the terms and conditions or the rules pertaining to
														admission as prescribed by the competent authorities and
														admit that they are binding upon me legally and
														legitimately.
													</p>
												</div>
...
<c:url value="Student_Personal_Details_Action"
	var="Student_Personal_Details_Action" />
<form:form action="${Student_Personal_Details_Action}" method="POST"
	id="student_submit_form" name="student_submit_form"
	modelAttribute="eid3">
...
<script nonce="${cspNonce}">
	...
	$(document)
			.ready(
					function() {
						...
						var parm_st = $("#pers_permanent_state :selected")
								.text();
						$("#pers_permanent_district").val(
								pers_permanent_district);
						var parm_dest = $("#pers_permanent_district :selected")
								.text();

						var pers_permanent_address = pers_permanent_house_no
								+ " " + pers_permanent_village + " "
								+ pers_permanent_postoffice + " "
								+ pers_permanent_tehsil + " " + parm_dest + " "
								+ parm_st + " " + pers_permanent_lendmark + " "
						...
						var pres_dest = $("#pers_present_district :selected")
								.text();
						...
						var corre_address = corre_house_no + " "
								+ corre_village + " " + corre_postoffice + " "
								+ corre_tehsil + " " + corr_dest + " "
								+ corr_st + " " + corre_lendmark + " "
								+ corre_pincode;
						...
						// 		$('div#table_adm').empty();
						// 		document.getElementById("no_neet_data").style.display = "block";
						// 						if ($("#no_neet_data").hasClass("custom-d-none")) {
						// 					 $( "#no_neet_data").removeClass("custom-d-none");
						// 					 $( "#no_neet_data" ).addClass("d-block");
						// 				}
						...
						if (id != "") {
							...
							//getImagethumb(id);
							$("#p_id").val(id);
							var signaturephoto = "ViewRefImageFileDownload5?kmlId="
									+ id + "&kmlfildname=signature";
							$("img#upload_signature_doc_preview").attr('src',
									signaturephoto);

							var photographphoto = "ViewRefImageFileDownload5?kmlId="
	...
	function getEdu_Detail(p_id) {
		//	alert("pid------->   "+p_id);
		$.post("getEdu_Detail_Ctrl?" + key + "=" + value, {
			p_id : p_id
		}, function(j) {
			strdata = "";
			//var x= 0;
			...
			if (j.length > 0) {
				for (var i = 0; i < j.length; i++) {
					strdata += '<tr>' + '<td><p>' + j[i][0] + '</p></td>'
							...
							+ '</p></td>' + '<td><p>' + j[i][4] + '</p></td>'
							+ '<td><p>' + j[i][5] + '</p></td>' + '<td><p>'
	...
	function FinalSubmit() {
		...
		// 	    var lds =  $("#late_admission_status").val();
		// 		if ( parseInt(lds) == 1 && $("#court_order").val() == "" &&  $("#court_order_hid").val() == "0" ) {
