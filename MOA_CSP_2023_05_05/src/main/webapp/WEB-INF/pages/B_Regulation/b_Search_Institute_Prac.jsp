          ...
          <div class="row align-items-center">
            <div class="col-md-6">
              <div class="title mb-30">
                <h2>INSTITUTE SEARCH FOR PRACTITIONER</h2>
              </div>
            </div>
            <!-- end col -->
            <div class="col-md-6">
        ...
        <div class="search-b-regulation-wrapper">
          ...
          <div class="col-lg-12">
              ...
              <div class="card-style mb-30">
                <h6 class="mb-25">INSTITUTE SEARCH FOR PRACTITIONER </h6>
               <div class="row">
                ...
                <div class="select-style-1">
                  ...
                  <select name="institute_status" id="institute_status" class="form-control customselect" onchange="statusChange()">						 
                    ...
                    <ul class="buttons-group mainbtn">
                      <li>
<!-- 	                    <a href="#0" class="main-btn primary-btn-outline btn-hover btn-iconic-icon" id="btn-reload"><i class="lni lni-search-alt"></i>search</a> -->
                  ...
                  <li>
                    <a href="edu_search_b_reg_url" class="main-btn dark-btn n btn-hover btn-clear" value="Reset">Reset</a>
                  </li>
...
<script>
	...
	function removecol() {	
    var tble = document.getElementById('Search_b_regulation_Master'); 
    var row = tble.rows;  
    var i = 1; 
...
//  		  var place_of_working = $("#place_of_working").val();
//  	 	  var  registration_for_type = $("select#registration_for_type").val();
//  	 	  var per_district = $("#per_district").val();
//  		  var per_state = $("#per_state").val();
//  		 var aadhaar_no = $("#aadhaar_no").val();
 		 
		$.post("getFilter_Edu_b_Reg_master_data?" + key + "=" + value, {
			startPage : startPage,
...
// 			}
 				...
 				jsondata.push([ j[i].ser ,j[i].chekboxaction,j[i].first_name, gender, j[i].img, j[i].father_name,
				    ...
				    $("#apprvchk").hide();
					  ...
					  jsondata.push([ j[i].ser  ,j[i].chekboxaction,j[i].first_name, gender, j[i].img, j[i].father_name,
							 ...
							 j[i].per_district, j[i].per_state, j[i].per_pincode, 
							 j[i].email_id , j[i].dob, j[i].nationality,j[i].vd, j[i].vioch,
		 					 j[i].reg_no, j[i].date_of_reg, j[i].reg_auth
		 						  ]);
				 }
		...
		$.post("getTotalEdu_b_Reg_master_dataCount?" + key + "=" + value, {
			//nrh_en_no:nrh_en_no,
				Search : Search,
			first_name:first_name,
			institute_status:institute_status,
	...
	function setApproveStatus(){
		...
		if(a == ""){
			alert("Please Select the Data for Approval"); 
	 			...
	 			$.post("Approve_b_regulation_Data?"+key+"="+value, {a:a,status:"1"}).done(function(j) {
	 			alert(j);
	 			location.reload();
	...
	function setRejectStatus(){
		...
		if(a == ""){
			alert("Please Select the Data for Reject"); 
				...
				$.post("Reject_b_regulation_Data?"+key+"="+value, {a:a,status:"2"}).done(function(j) {
					alert(j);
					location.reload();
				Search();
			}); 
