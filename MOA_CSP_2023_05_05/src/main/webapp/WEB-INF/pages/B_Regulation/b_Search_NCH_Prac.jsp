...
<!--                 </div> -->
               ...
               <div class="select-style-1">
                  ...
                  <li>
                    <a href="b_Search_NCH_pracUrl" class="main-btn dark-btn n btn-hover btn-clear" value="Reset">Reset</a>
                  </li>
                  <li>
                    <input type="button" class="main-btn success-btn  btn-hover"  id="apprvbt" style="display: none"  value="Approve" onclick="return setApproveStatus();">
                  </li>
                  <li>
                    <input type="button" class="main-btn danger-btn btn-hover" id="rejbt"  style="display: none" value="Reject" onclick="return setRejectStatus();">
                  </li>
        ...
        <div class="card-style mb-30 selectsection" id="checkheaddiv">
			 ...
			 <input  type="hidden" id="CheckVal" name="CheckVal">
				  <input class="form-check-input" type=checkbox id='nSelAll' name='tregn' onclick='setselectall();'> <label class="form-check-label">Select all [<span id="tregn">0</span><span id='nTotRow1'>/</span><span id="tregnn"  >  </span></label>]
					</div>
              ...
              <div class="col-lg-12">
                <div class="card-style mb-30">
                  ...
                  <div class="table-wrapper table-responsive custom-datatable-p">
                    <table id="b_Search_NCH_Prac" class="table">
                      <thead>
                        <tr>
...
function setselectall(){
	...
	$(document).ready(function() {
		...
		mockjax1('b_Search_NCH_Prac');
		table = dataTable('b_Search_NCH_Prac');
	...
	function removecol() {	
    var tble = document.getElementById('b_Search_NCH_Prac');
		//tble.clear();
    var row = tble.rows;  
    var i = 1; 
	...
	function data(b_Search_NCH_Prac) {
		//debugger;
		jsondata = [];
		var table = $('#' + b_Search_NCH_Prac).DataTable();
...
// 		var currentPage = info.page;
		...
		$.post("getFilter_b_NCH_Prac_data?" + key + "=" + value, {
			startPage : startPage,
			...
			if(gender == "2"){
			 ...
			 if(institute_status=='0'){
				...
				jsondata.push([ j[i].ser ,j[i].chekboxaction, j[i].nrh_en_no,
				 ...
				 $("#checkheaddiv").hide();
					...
					jsondata.push([ j[i].ser ,j[i].chekboxaction, j[i].nrh_en_no,
						...
						j[i].per_district, j[i].per_state, j[i].per_pincode ,
						j[i].email_id
						, j[i].dob, j[i].nationality,j[i].vd, j[i].vioch
 						  ]);
			 }
		...
		$.post("getTotal_b_NCH_Prac_dataCount?" + key + "=" + value, {
			//nrh_en_no:nrh_en_no,
			first_name:first_name,
			institute_status:institute_status,
			 gender:gender,
	...
	function findselected(){
		...
		if(a == ""){
			alert("Please Select the Data for Approval"); 
 				...
 				$.post("Approve_NCH_b_regulation_Data?"+key+"="+value, {a:a,status:"1"}).done(function(j) {
	...
	function setRejectStatus(){
		...
		if(a == ""){
			alert("Please Select the Data for Reject"); 
				...
				$.post("reject_NCH_b_regulation_Data?"+key+"="+value, {a:a,status:"2"}).done(function(j) {
