								...
								<h6 class="mb-10">Seat Details</h6>
									...
									id="btnExport"><i class="lni lni-printer" value="EXCEL" 
 										title="Export to EXCEL"></i> EXCEL </a></li>
 											...
 											<input type="hidden" name="excel_data" id="excel_data" value="0" />
 										 											<input type="hidden" name="maxResult" id="maxResult" value="0" />
...
$(document).ready(function() {
	...
	var exc = $("#excel_data").val();

	
	
	if(exc != null && exc != "0"){
		$("#data1").val(exc.split("|")[0]);
		$("#obj1").val(exc.split("|")[1]);
...
$("#state_auto_id").on("input", function(){
		...
		if(("${rolename}" != "State_Council_NCISM") && ("${rolename}" != "State_Council_NCH") && ("${rolename}" != "University_NCISM") && ("${rolename}" != "University_NCH") && ("${rolename}" != "Institute_NCISM") && ("${rolename}" != "Institute_NCH"))
		{
	...
	$("#uni_auto_id").on("input", function(){
		if(("${rolename}" != "University_NCISM") && ("${rolename}" != "University_NCH") && ("${rolename}" != "Institute_NCISM") && ("${rolename}" != "Institute_NCH"))
	{
	...
	$("#ins_auto_id").on("input", function(){
		if(("${rolename}" != "Institute_NCISM") && ("${rolename}" != "Institute_NCH"))
		{
	...
	function statecount(obj){
		var maxResult=0;
		var max_state_id=0;
		$.post('e_formgetDashBoardDataReload3?' + key + "=" + value,{obj:obj},function(k) {
			...
			for(var i=0;i<k.length;i++){
				...
				console.log("ABCD-----"+q_type+"=========="+state_id);
						...
						if (q_type.split("-").length > 0) {
							...
							if(maxResult < parseInt(q_type_sub[1])){
								maxResult = parseInt(q_type_sub[1]);
	...
	function setOnClick(){
		...
		for(var i = 1 ; i <= "${getSystemList.size()}"; i++){
		 		...
		 		obj = obj.substring(0,obj.length-1);
		            ...
		            e_formgetDataReloadUniversity(t,obj);
//		             statecount(t);
...
//	 		getUniversityFromState(mainid);
	...
	function UpdateAllRecords(obj){
		...
		$.post('e_formgetDashBoardDataMainReload?' + key + "=" + value,{obj:obj},function(data) {
	 ...
	 if(alloted > 0){
		 $("#count1").text(alloted);
//			 counter("count1", alloted, 1); //ALLOTED
	 }
		else
			 $("#count1").text(0);
	 	
		if(available > 0){
			 $("#count2").text(available);
//			 counter("count2", available, 1); //AVAILABLE
		}
				 else
					 $("#count2").text(0);
		if(total > 0){
			 $("#count3").text(total);
//			 counter("count3", total, 10); //TOTAL
		}
		 else
			 $("#count3").text(0);
...
//					 $("#count6").text(lasttotalp);
				 $("#count6").text(Math.round(lasttotalp));
//					 counter("count6", Math.round(lasttotalp), 1); //AVRAGE TOTAL UP
				}
			 else{
...
//					 $("#count6").text(lasttotalp);
				 $("#count6").text(Math.round(lasttotalp));
//						 counter("count6", Math.round(lasttotalp), 1); //AVRAGE TOTAL DOWN
			 }
		...
		function e_formgetDataReloadUniversity(data,obj){
			$("#excel_data").val(data+"|"+obj);
		
			console.log(data+"--------"+obj);
			$
			.post(
					"e_formgetDashBoardDataReload?" + key + "=" + value,
					{
						data:data,obj : obj
					},
					function(j) {
...
// 	$.post('e_formcallDefaultSelectDashboard?' + key + "=" + value,{},function(k) {
...
// 		    e_formgetDataReloadUniversity("S,"+k[0].state_id,obj);
...
// 			    e_formgetDataReloadUniversity("S,"+state,obj);
// }
// 	});
	var state_id = $("#maxResult").val();
	 populateGraph("S,"+state_id,obj);
	    populateGraph2("S,"+state_id,obj);
	    e_formgetDataReloadUniversity("S,"+state_id,obj);
	 $("#chatDivLab").text($("#state_id"+state_id).text());
	 $("#chatDivLab2").text($("#state_id"+state_id).text());
...
function populateGraph(data,obj){
	...
	$.post('e_formgetDashBoardDataReload?' + key + "=" + value,{data:data,obj:obj},function(k) {
		console.log(k);
		xAxis.data.setAll(k);
		$("#data1").val(data);
		$("#obj1").val(obj);
...
function populateGraph2(data,obj){
	...
	$.post('e_formgetDashBoardDataReload2?' + key + "=" + value,{data:data,obj:obj},function(k) {
// 		alert(k);
...
function getUniversityFromState(state_id){
	...
	$.post(
			"e_formgetUniverCityListBystate?" + key + "=" + value,
			{
				state_id:state_id
			},
...
function getInstituteFromUniversity(university_id){
	...
	$.post(
			"e_formgetInstituteListByuniversity?" + key + "=" + value,
			{
