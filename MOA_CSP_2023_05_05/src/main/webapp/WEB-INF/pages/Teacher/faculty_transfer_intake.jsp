				...
				<div class="breadcrumb-wrapper mb-30">
					<nav aria-label="breadcrumb">
						<ol class="breadcrumb">
							...
							<li class="breadcrumb-item active" aria-current="page">Faculty Transfer Intake</li>
						</ol>
					</nav>
				</div>
			</div>
                ...
                <form:form name="" id="" action="Search_Student_RegAction" method="post" class="form-horizontal" commandName="Search_Student_Reg_CMD"> 
						...
						<div class="row">
							...
							<div class="col-12 col-sm-12 col-md-6 col-lg-4">
								<div class="input-style-1">
									...
									<input type="text" id="ayush_id" name="ayush_id" placeholder="Enter Ayush Id" class="form-control"
										autocomplete="off"   />
								</div>							
								<!-- end select -->
							</div>
                 ...
                 <div class="col-12 col-sm-12 col-md-6 col-lg-4">
                	<div class="input-style-1">
                 	...
                 	<input type="text" name="teacher_code" id="teacher_code" class="autocomplete xt-transupp"  placeholder="Enter Teacher Code"  maxlength="100">
                 	</div>
                 	  	<input type="hidden" id="teacher_code_auto"
										name="teacher_code_auto" value="0" autocomplete="off" />
               ...
               <div class="col-12 col-sm-12 col-md-6 col-lg-4">
					...
					<div class="col-12 col-sm-12 col-md-6 col-lg-4">
						...
						<ul class="buttons-group mainbtn">
							...
							<li>
								<a href="teacher_transfer_url" class="main-btn dark-btn n btn-hover" type="button">Reset</a>
							</li>
...
<!-- 										<li><a  class="main-btn success-btn btn-hover btn-iconic-icon btnexcel" id="btnExport"><i class="bi bi-file-earmark-excel"></i>Excel</a></li> -->
										
								<li><a id="update_institute" href="#" class="main-btn deactive-btn btn-hover btnupda">Update Institute</a></li>		
						</ul>
						
						<br>
						
						<div class="card-style mb-0" id="note">
						
						<div class="row">
                                  
                                        <div class="col-12 d-flex " >
                                            <h5 class="mb-2" > <input type="checkbox" id="checkbox1" name="checkbox1"><label for="text-input" class="co d-inline"></label>
                                              </h5>
                                              
                                            <ol>
                                            
                                           <li class="mandatory">Are You Sure You Want to update this Institute </li>
<!-- 											  <li>I hereby confirm that the information provided herein is accurate, correct and complete and that the documents submitted along with this application form are genuine. -->
<!-- 											  	  I understand and agree that this declaration is final and irrevocable, and that it is not subject to cancellation .</li> -->
											</ol> 
...
$(document).ready(function() {
	...
	$('#btn-reload').on('click', function() {
		table.ajax.reload();
		if ($("#getSearch_teacher").hasClass("d-none")) {
			 $( "#getSearch_teacher").removeClass("d-none")
				}
		if ($( "#getSearch_teacher_info").hasClass('d-none')) {
			$( "#getSearch_teacher_info").removeClass("d-none")
	}
	});
	
	if (!$( "#getSearch_teacher").hasClass('d-none')) {
		$( "#getSearch_teacher").addClass("d-none")
}
	if (!$( "#getSearch_teacher_info").hasClass('d-none')) {
		$( "#getSearch_teacher_info").addClass("d-none")
}
	...
	if (!$( "#update_institute").hasClass('d-none')) {
		$( "#update_institute").addClass("d-none")
}
...
function data(getSearch_teacher) {
	...
	$.post("getFilterfaculty_transfer_data?" + key + "=" + value, {
		startPage : startPage,
		...
		for (var i = 0; i < j.length; i++) {
			...
			jsondata.push([ j[i].ser,j[i].chekboxaction,j[i].ayush_id,j[i].teacher_code,j[i].first_name,j[i].university_name,j[i].institute_name,
	...
	$.post("DataTablefaculty_transfer_DataTotalCount?" + key + "=" + value, {
...
// 	};
	...
	document.getElementById('update_institute').onclick = function() {
		 UpdateInstitute();	
	};
...
// 	};
	...
	$('.nrCheckBox').on('click', function() {
		//$('.nrCheckBox').click(function(){
		    if($(this).is(':checked')){
	    	$( "#update_institute").removeClass("d-none");	 
		    } else {
		    	$( "#update_institute").addClass("d-none");
...
function UpdateInstitute(){
	...
	var chkval=$("#CheckVal").val();
	
	
	$.post("faculty_transfer_inst_id?"+key+"="+value, {chkval:chkval }).done(function(j) {
		
//			alert("Student's Admission is Approved for Edit.");
			alert("Faculty Transfer Completed");
	...
	// $( "#note").removeClass("d-none");
		...
		//AUTOCOMPLETE ON TEACHER CODE
		function AutoCompTeachercode() {
			debugger;
			var susval = [];
			var myResponse1 = [];
			
			var susNoAuto = $("#teacher_code");
			
			var teacher_code_auto=($("#teacher_code_auto").val());

			susNoAuto.autocomplete({
				source : function(request, response) {
					$.ajax({
						type : 'POST',
						url : "getTeachercodeAuto?" + key + "=" + value,
						data : {
							autoString : $("#teacher_code").val()
//	 						degree_cat : $("#degree_id option:selected").text()
						},
						success : function(data) {
							
							for (var i = 0; i < data.length; i++) {
								susval.push(data[i].teacher_code);
				...
				select : function(event, ui) {
					...
					document.getElementById("teacher_code_auto").value = myResponse1[$.inArray(ui.item.value,susval)];
				}
			});
		}
