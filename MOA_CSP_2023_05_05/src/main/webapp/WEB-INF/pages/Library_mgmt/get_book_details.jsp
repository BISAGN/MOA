					...
					<div class="breadcrumb-wrapper mb-30">
						<nav aria-label="breadcrumb">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active" aria-current="page">Book all Details</li>
							</ol>
						</nav>
					</div>
				</div>
					...
					<div class="table-wrapper table-responsive custom-datatable-p">
						<table class="table" id="search_system">
							<thead> 
								...
								<th><h6>Book Id</th>
									<th><h6>Book Name</th>
									<th><h6>Date of Published</th>
									<th><h6>Is Book Available?</th>
 									<th><h6>Action</h6></th>
...
function setTimeLoadForTable(){
	...
	document.getElementById('book_name').onkeypress = function() {
	 ...
	 document.getElementById('date_of_published').onkeypress = function() {
		return Validation();
	}; 
...
function data(search_system) {
	...
	var date_of_published = $("#date_of_published").val(); 
	var is_book_available = $("#is_book_available").val(); 



	$.post("getFilterbook_details_Master_data?" + key + "=" + value, {
		startPage : startPage,
		...
		orderType : orderType,
		book_name : book_name,
		date_of_published : date_of_published,
		is_book_available : is_book_available,


 	}, function(j) {
		for (var i = 0; i < j.length; i++) {
			jsondata.push([j[i].ser, j[i].book_name,j[i].date_of_published,j[i].j[i].date_of_published,j[i].action]);
		}
	});
 	$.post("getTotalbook_details_Master_dataCount?" + key + "=" + value, {
 		Search : Search,
 		book_name  : book_name ,
 		date_of_published :date_of_published,
 		is_book_available : is_book_available,
...
function Search() {
	$("#book_name1").val($('#book_name').val());
 	$("#date_of_published1").val($('#date_of_published').val()); 
...
function Validation() {
		if ($("#book_name").val() =="" ) {
		 ...
		 if ($("#date_of_published").val() =="" ) {
			alert("Please Enter Date of Published.");
			$("input#date_of_published").focus();
			return false;
		} 
