		...
		<div class="form-elements-wrapper">
			<div class="row">
				<div class="col-lg-12 col-md-12 col-sm-12">
					<!-- input style start -->
					<form:form name="nch_define_topic_wise_marks" id="nch_define_topic_wise_marks" action="nch_define_topic_wise_marksAction"
						method="post" modelAttribute="nch_define_topic_wise_marksCMD">
...
<!-- 											<label id="topic_name"> Topic (0)<span class="mandatory">*</span></label>  -->
<!-- 												<input type="text" -->
<!-- 												id="search_data"  -->
<!-- 												autocomplete="off" placeholder="Search Topic"> -->
<!-- 										</div> -->
...
<!-- 										<label id="system_name">System (0)<span class="mandatory">*</span></label> -->
									...
									<div class="col-12 mb-30" id="checkboxes">
										<c:forEach var="item" items="${get_NCH_TopicListbyCourse}" varStatus="num">
							...
							<li id="btn-reload1"> <a href="Nch_define_topic_wise_mark_url" class="main-btn dark-btn-outline btn-hover btn-iconic-icon btnback">
								...
								<li  id="btn-reload2"><a href="Nch_define_topic_wise_mark_url"
			...
			<div class="col-lg-12 col-md-12 col-sm-12">
				<div class="card-style mb-30">
					<div class="table-wrapper table-responsive custom-datatable-p">
						<table class="table" id="search_Nch_define_topic_wise_marks">
							<thead>
								<tr>
...
<c:url value="delete_nch_define_topic_wise_mark_Url" var="deleteUrl" /> 
...
$(document).ready(function() {
	...
	mockjax1('search_Nch_define_topic_wise_marks');
	table = dataTable('search_Nch_define_topic_wise_marks');
	...
	if ('${list.size()}' == "") {
		$("div#search_Nch_define_topic_wise_marks").hide();
	}
			...
			.done(
					function(data) {
						...
						for (var i = 0; i < data.length; i++) {
							...
							+'id="multisub'+ data[i][0] + '" value="' + data[i][0] +'"> '+ data[i][1] + '</label>';
						}
...
document.addEventListener('DOMContentLoaded', function() {
	...
	document.getElementById('btn-save').onclick = function() {
		if(Validation()){
			 $("#nch_define_topic_wise_marks").submit();
		}
	};
...
function data(search_Nch_define_topic_wise_marks) {
	
	jsondata = [];
	var table = $('#' + search_Nch_define_topic_wise_marks).DataTable();
...
// 	var topic_name = $("#old_course_topic").val();
	$.post("getFilterNch_define_topic_wise_marks_data?" + key + "=" + value, {
		startPage : startPage,
	...
	$.post("getTotalNch_define_topic_wise_marks_dataCount?" + key + "=" + value, {
		Search : Search,
		system_id:system_id,
		degree_id : degree_id,
		course_id : course_id,
...
function editData(id,system_id,degree_id,professional_id,course_id,marks,topic_id,cids) {
	...
	$("select#topic_name").val(topic_id);
// 	alert(topic_id)
// 	checkoptioninedit(topic_id);
	$("#Edit_ids").val(id);
...
// 	}
	...
	$.post('updateNch_define_topic_wise_marksAction?' + key + "=" + value, {
		id : id,
		system_id:system_id,
		degree_id : degree_id,
		professional_id : professional_id,
...
function checkoptioninedit(sids){
	...
	var tidsarr = [];
	var tidsarr = sids.split(':');
	
	for(var i=0;i<tidsarr.length;i++){
		$("#multisub"+tidsarr[i]).click();
	}
...
// 					$("div#add_course_topic").append(
// 						'<input class="multi form-check-input mr-5" type="checkbox" id="topic_name'+q+'" name="topic_name" value="'+j[p][0]+'"/>'
...
// ($('#topic_name' + i).val());
// 				var temp_data = $('#topic_name' + i).val();
// 				if(variable.trim() == temp_data.trim()){
// 					$('#topic_name' + i).click();
// 				}
// // 	    		if('${topic_id}'.split(",")[i]=='1')
