			...
			<div class="row align-items-center">
				<div class="col-lg-6 col-md-6 col-sm-12 col-12">
					<div class="title mb-30">
						<h2>
							<span id="lbladd1"></span>Add Non Lecture Activities
						</h2>
					</div>
				</div>
				<!-- end col -->
					...
					<div class="breadcrumb-wrapper mb-30">
						<nav aria-label="breadcrumb">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active" aria-current="page">Add Non Lecture Activities</li>
							</ol>
						</nav>
					</div>
				</div>
		...
		<div class="form-elements-wrapper">
			<div class="row">
				<div class="col-lg-12 col-md-12 col-sm-12">
					<!-- input style start -->
					<form:form name="Nch_Add_Non_Lecture_Activities"
						id="Nch_Add_Non_Lecture_Activities"
						action="Nch_Add_Non_Lecture_ActivitiesAction" method='POST'
						commandName="Nch_Add_Non_Lecture_ActivitiesCMD"
						...
						<div class="card-style mb-30">
										...
										<div class="table-wrapper table-responsive custom-table  b-top mt-0">
											...
											<thead>
												<tr>
													...
													<th rowspan="2"><h6>No. Of Hours</h6></th>
											...
											<tbody id="att_Tbbody">
												<tr id="tr_id_att">
													...
													<td class="min-width">
														<div class="input-style-1">
															<input type="text" id="no_of_hours1"
																name="no_of_hours1" maxlength="3"
																class="form-control" autocomplete="off"
							...
							<li><a id="btn-reload"class="main-btn secondary-btn btn-hover btn-iconic-icon btnsearch"
								...
								<li><a href="CC_Nch_Add_Non_Lecture_Activities_url"
			...
			<section class="single-detail-block">
				<div class="tables-wrapper">
					<div class="row" id="pop">
						<div class="col-lg-12">
							<div class="card-style mb-30">
								<h6 class="mb-10">Add Non-Lecture Activities Subject</h6>
									...
									<table class="table" id="pop">
										<thead>
											<tr>
												...
												<th class="middle-center"><label class="ml-5 bold">No
														of hours<span class="mandatory"
														maxlength="3">*</span>
												</label></th>
			...
			<div class="modal fade custom-modal bd-example-modal-lg"
				...
				<div class="modal-dialog modal-lg modal-dialog-scrollable">
					...
					<div class="modal-header">
							<h3 class="modal-title">Add Non Lecture T-L Method</h3>
...
<c:url value="Edit_Nch_Add_Non_Lecture_Activities_Url"
	var="Edit_Nch_Add_Non_Lecture_Activities_Url" />
<form:form action="${Edit_Nch_Add_Non_Lecture_Activities_Url}" method="post"
	id="editForm" name="editForm" modelAttribute="id1">
	<input type="hidden" name="id1" id="id1" />
</form:form>

<c:url value="deleteNCH_Non_Lecture_ActivitiesUrl"
	var="deleteUrl" />
<form:form action="${deleteUrl}"
	...
	$("#att_Tb").show();
		 ...
		 if(att < 51){
				 ...
				 $("table#att_Tb").append('<tr align="center" id="tr_id_att'+att+'"><td>'+att+'</td>'
					 ...
					 +'<td class="min-width"><div class="input-style-1"><input type="text" id="no_of_hours'+att+'" name="no_of_hours'+att+'"  maxlength="3" class="form-control"  autocomplete="off" placeholder="No. Of Hours" ></div></td>'
...
document.addEventListener('DOMContentLoaded', function() {
	...
	document.getElementById('btn-save').onclick = function() {
		if(Validation()){
			 $("#Nch_Add_Non_Lecture_Activities").submit();
		}
	};
	...
	document.getElementById('no_of_hourse1').onkeypress = function() {
...
function data(search_Add_Non_Lecture_Activities) {
	...
	$.post("getFilterNch_Add_Non_Lecture_Activities_data?" + key + "=" + value, {
		startPage : startPage,
	...
	$.post("getTotalNch_Add_Non_Lecture_Activities_dataCount?" + key + "=" + value, {
		system_id:system_id,
		degree_id:degree_id,
		professional_id:professional_id,
		course_id:course_id,
...
function addOnclick(index){
	...
	document.getElementById('no_of_hours'+index).onkeypress = function() {
		return isNumberKey0to9(event,this);
	};
}
...
function Pop_Up_view_child_data(hid) {
	...
	$('tbody#att_TbbodyNameMed').empty();
		$
				.post( 
						"Nch_Add_Non_Lecture_Activities_ChildUrl?" + key
								+ "=" + value,
						{
							hid : hid
						},
...
function getpop() {
	...
	var course_id = $("#course_id").val();
	var temp_count = 0;
	$.post("getNch_AddNonLectureActivitiesviewdata?"+key+"="+value,{course_id:course_id},function(j) {
		for(var i=0;i<j.length;i++){
// 			paper = j[i][4];
// 			if(paper == "PAPER I"){
				$("table#pop").append('<tr>' 
							+'<td><label id="sr_no">'+j[i][0]+'</label></td>'
							+'<td><label id="non_lecture_Teaching_Learning_methods">'+j[i][1]+'</label></td>'
							+'<td><label id="no_of_hours">'+j[i][2]+'</label></td>'
							+'</tr>');
				temp_count += parseInt(j[i][2]);
// 			}
		}
		$("table#pop").append('<tr>' 
				+'<td><label id="sr_no"></label></td>'
				+'<td><label id="non_lecture_Teaching_Learning_methods"></label></td>'
				+'<td><label id="no_of_hours">'+temp_count+'</label></td>'
				+'</tr>');
  });
...
// 			if(paper == "PAPER I"){
				$("table#pop").append('<tr>' 
							+'<td><label id="sr_no1"></label></td>'
							+'<td><label id="non_lecture_Teaching_Learning_methods1">Practical (refer Table 4)</label></td>'
							+'<td><label id="no_of_hours1">'+j[i][0]+'</label></td>'
							+'</tr>');
// 			}
		...
		$("table#pop").append('<tr>' 
				+'<td><label id="sr_no1"></label></td>'
				+'<td><label id="non_lecture_Teaching_Learning_methods1">Total</label></td>'
				+'<td><label id="no_of_hours1">'+temp_count1+'</label></td>'
				+'</tr>'); 
