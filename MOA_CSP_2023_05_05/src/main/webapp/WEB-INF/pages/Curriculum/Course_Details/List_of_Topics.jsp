			...
			<div class="row align-items-center">
				<div class="col-lg-6 col-md-6 col-sm-12 col-12">
					<div class="title mb-30">
						<h2>
							<span id="lbladd1"></span>T2 - Add Content of Subject
						</h2>
					</div>
				</div>
				<div class="col-lg-6 col-md-6 col-sm-12 col-12">
					<div class="breadcrumb-wrapper mb-30">
						<nav aria-p="breadcrumb">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active" aria-current="page">T2 -
									Add Content of Subject</li>
							</ol>
						</nav>
					</div>
				</div>
				...
				<div class="col-lg-12 col-md-12 col-sm-12">
					...
					<form:form name="list_topics" id="list_topics"
						...
						<div class="custom-field-block">
							<h6 class="mb-25">T2 - Add Content of Subject</h6>
							<div class="row">
								...
								<div class="col-12 col-sm-12 col-md-6 col-lg-4">
									<div class="select-style-1">
										...
										<div class="select-position">
											<select class="singleselect form-control form-control-lg" name="paper_id" id="paper_id">
												<option value="0">--Select--</option>
										...
										<table class="table" id="att_Tb">
											<thead>
												<tr>
													...
													<th rowspan="2"><h6>
															Lecture Hours<span class="mandatory">*</span>
														</h6></th>
													<th rowspan="2"><h6>
															Non Lecture Hours<span class="mandatory">*</span>
														</h6></th>
											...
											<tbody id="att_Tbbody">
												<tr id="tr_id_att">
													...
													<td>
														<div class="select-style-1">
															<div class="select-position">
																<select class="form-control" name="topic_id1" id="topic_id1">
																	<option value="0">--Select--</option>
													...
													<td>
														<div class="select-style-1">
															<div class="select-position">
																<select class="form-control" name="term_id1" id="term_id1">
																	<option value="0">--Select--</option>
...
<%-- 																	</c:forEach> --%>
													...
													<td class="min-width">
														<div class="input-style-1">
															<input type="text" id="lecture_hours1"
																name="lecture_hours1" maxlength="3" class="form-control"
																autocomplete="off" placeholder="Lecture Hours">
														</div>
													</td>
													<td class="min-width">
														<div class="input-style-1">
															<input type="text" id="non_lecture_hours1"
																name="non_lecture_hours1" maxlength="3"
																class="form-control" autocomplete="off"
																placeholder="Non Lecture Hours">
														</div>
													</td>
													<td>
		...
		<section class="single-detail-block">
			...
			<div id="pop">
						...
						<div class="custom-multi-table">
							<div class="card-style mb-30">
								<h6 class="mb-10">Contents of Subject</h6>
								<div class="table-wrapper table-responsive custom-table custom-table-v2">
		
							<table class="table table-striped" id="pop1">
								<thead>
									<tr>
										<th><h6>Sr No.</h6></th>
										<th><h6>A2 List of Topics</h6></th>
										<th><h6>B2 Term</h6></th>
										<th><h6>C2 Marks</h6></th>
										<th><h6>D2 Lecture hours</h6></th>
										<th><h6>E2 Non Lecture hours</h6></th>
									</tr>
								</thead>
								<tbody></tbody>
...
<script nonce="${cspNonce}" type="text/javascript">
	...
	function getpop() {
		...
		var topic = "";
		 ...
		 $("table#pop1 > tbody").empty();
		$.post("getView_T2_data?" + key + "=" + value, {
			course_id : course_id
		}, function(j) {
			for (var i = 0; i < j.length; i++) {
				topic = j[i][1];
				var sta = j[i][2].split('/');
				for (var s = 0; s < sta.length; s++) {
					if (s == 0) {
						topic += "<br>" + " * " + sta[s];
					} else {
						topic += "\n" + " * " + sta[s];
				...
				if (i == 0) {
					...
					$("table#pop1").append(
							'<tr><td colspan="6"><p id="paper1TR">'
									+ j[i][7] + '</p></td></tr>'
									+ '<tr><td><p id="sr_no">' + j[i][0]
									+ '</p></td>'
									+ '<td><p id="topic">' + topic
									+ '</p></td>' + '<td><p id="term">'
									+ j[i][3] + '</p></td>'
					...
					if (paper != j[i][7]) {
						paper = j[i][7];
						$("table#pop1").append(
								'<tr><td colspan="10"><p id="paper1TR">'
										+ j[i][7] + '</p></td></tr>'
										+ '<tr><td><p id="sr_no">'
										+ j[i][0] + '</p></td>'
										+ '<td><p id="topic">' + topic
			...
			$("table#att_Tb")
					.append(
							'<tr align="center" id="tr_id_att'+att+'"><td>'
									+ att
									+ '</td>'
									+ '<td><div class="select-style-1"><div class="select-position"><select class="form-control" id="topic_id'+att+'" name="topic_id'+att+'"  ><option value="0">--Select--</option>'
									// 					 +'<c:forEach var="item" items="${getTopicList}" varStatus="num"><option value="${item.id}" name="${item.topic}">${item.topic}</option></c:forEach>' 
									+ '</select></div></div> </td>'
									+ '<td><div class="select-style-1"><div class="select-position"><select class="form-control" id="term_id'+att+'" name="term_id'+att+'" ><option value="0">--Select--</option>'
// 									+ '<c:forEach var="item" items="${geti3_termList}" varStatus="num"><option value="${item.id}" name="${item.term}">${item.term}</option></c:forEach>'
									+ '</select></div></div> </td>'
									+ '<td class="min-width"><div class="input-style-1"><input type="text" id="lecture_hours'+att+'" name="lecture_hours'+att+'"  maxlength="3" class="form-control"  autocomplete="off" placeholder="Lecture Hours" ></div></td>'
									+ '<td class="min-width"><div class="input-style-1"><input type="text" id="non_lecture_hours'+att+'" name="non_lecture_hours'+att+'"  maxlength="3" class="form-control"  autocomplete="off" placeholder="Non Lecture Hours" ></div></td>'
