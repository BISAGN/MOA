			...
			<div class="row align-items-center">
				<div class="col-lg-6 col-md-6 col-sm-12 col-12">
					<div class="title mb-30">
						<h2>
							<span id="lbladd">Update Nch Student Lecture Plan</span>
						</h2>
					</div>
				</div>
				<!-- end col -->
					...
					<div class="breadcrumb-wrapper mb-30">
						<nav aria-label="breadcrumb">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active" aria-current="page">Update Nch
		...
		<div class="form-elements-wrapper">
			<div class="row">
				<div class="col-12 col-sm-12 col-md-12 col-lg-12">
					<!--  style start -->
					<form:form name="edit_nch_student_lecture_planUrl"
						id="edit_nch_student_lecture_planUrl" method="post"
						class="form-horizontal" modelAttribute="Nch_Student_LecturerCMD">
...
<!-- 														<th rowspan="2"><h6>Sub-Topic</h6></th> -->
												...
												<tbody id="family_sibtbody">
													...
													<!-- 														<div class=""> -->
													<!-- 															<div class="input-style-1"> -->
													<!-- 															<select name="learning_outcome" id="learning_outcome" class="form-control" > -->
									...
									<div class="row">
										<div class="col-lg-12 col-md-12 col-sm-12">
											<ul class="buttons-group mainbtn">
												<li><a href="Search_Nch_Student_Lecturer_PlanUrl"
...
// 		var sub_topic = $("#sub_topic").val();
		...
		var sdate = $("#sdate").val();
		var form = $("#edit_nch_student_lecture_planUrl").serialize();
		$
				.post(
						"getNchStudent_DetailsforEdit?" + key + "=" + value,
						form,
						function(j) {
...
//				 		    	$("#Migrated_Students_remove"+ps).show(); 
								...
								window.location.href = "Search_Nch_Student_Lecturer_PlanUrl";
							}
...
// 		var sub_topic = $("#sub_topic").val();
		...
		$.post("getFilter_Nch_Student_Lecturer_data?" + key + "=" + value, {
			startPage : startPage,
...
// 			sub_topic : sub_topic,
			...
			for (var i = 0; i < j.length; i++) {
				jsondata.push([ j[i].sr_no, j[i].professional,
					j[i].course_name, j[i].faculty, j[i].topic_id,
					j[i].learning_outcome, j[i].instructional_method,
					j[i].sdate ]);
			}
		});
		$.post("getTotal_Nch_Student_Lecturer_dataCount?" + key + "=" + value, {
			Search : Search,
			professional : professional,
			course_name : course_name,
			faculty : faculty,
...
// 			sub_topic : sub_topic,
	...
	// 			return false;
	// 		}
	// 		if ($("#learning_outcome").val() =="0" ) {
	// 			alert("Please Enter learning_outcome.");
		...
		$
				.post(
						...
						function(j) {
							...
							$("select#topic_id" + ind).html(options);
...
// 		$
// 				.plearning_outcomeLearning_Objective1?" + key + "=" + value,
// 						{
...
// 	}
	...
	function Nchgetlearn_outListby_Topic(ind) {

	var topic_id = $("#topic_id" + ind).val();
	
			$.post('Nchgetlearn_outmeList1?' + key + "=" + value, {
				topic_id : topic_id
			},			
			function(j) {
				var options = '<option   value="0">' + "--Select--"
				...
				$("select#learning_outcome" + ind).html(options);
			});
	}
	...
	function getFacultyData(ind) {
		...
		$
				.post(
						"getNchfacutlydetails1?" + key + "=" + value,
						{
							course_id : course_id
						},
						function(j) {
	...
	function SelectTopicForUpdate(ind) {
		...
		document.getElementById('topic_id' + ind).onclick = function() {
			Nchgetlearn_outListby_Topic(ind);
// 			getSubTopic(ind);
		};
...
// 		$( "#sdate" ).datepicker( "option", "disabled", true );
		...
		$
				.post(
						'getNchLecturedetailsbyDateforEdit?' + key + "=" + value,
						{
							sdate : sdate,
							professional : professional
						},
								...
								$("table#family_table")
										.append(
												'<tr align="center" id="tr_id_att'+att+'"><td>'
														...
														+ '<td><div class=""><div class="select-style-1"><select name="topic_id'+att+'" id="topic_id'+att+'" class="form-control form-control-lg" ><option value="0">---Select---</option></select></div></div></td>'
...
// 														+ '<td><div class=""><div class="select-style-1"><select name="sub_topic'+att+'" id="sub_topic'+att+'" class="form-control form-control-lg" ><option value="0">---Select---</option></select></div></div></td>'
														...
														+ '<td><div class=""><div class="select-style-1"><select name="learning_outcome'+att+'" id="learning_outcome'+att+'" class="form-control form-control-lg" ><option value="0">---Select---</option></select></div></div></td>'
								...
								$("#topic_id" + att).val(k[i][4]);
								Nchgetlearn_outListby_Topic(att);
// 								getSubTopic(att);
// 								$("#sub_topic" + att).val(k[i][5]);
// 								getLearning(att);

								$("#learning_outcome" + att).val(k[i][5]);

								$("#instructional_method" + att).val(k[i][6]);

								$("#EditHid" + att).val(k[i][7]);
	...
	function Validation() {
		...
		for (i = 1; i <= parseInt(datacounthId); i++) {
			...
			if ($("#yes" + i).is(":checked")) {
				...
				if ($("select#topic_id" + i).val() == "0") {
					alert("Please Select Topic of Row " + i);
					$("select#topic_id" + i).focus();
					return false;
				}
...
// 				}
				...
				if ($("select#learning_outcome" + i).val() == "0") {
					alert("Please Select Learning Outcome of Row " + i);
					$("select#learning_outcome" + i).focus();
					return false;
				}
	...
	function getreadonlyupdatedataforno(att) {
		...
		$("#topic_id" + att).val("0");
		$("#topic_id" + att).attr('readonly', 'true');

// 		$("#sub_topic" + att).val("0");
// 		$("#sub_topic" + att).attr('readonly', 'true');

		$("#learning_outcome" + att).val("0");
		$("#learning_outcome" + att).attr('readonly', 'true');
	...
	function getreadonlyupdatedataforyes(att) {
		...
		$("#topic_id" + att).attr('readonly', false);

// 		$("#sub_topic" + att).attr('readonly', false);

		$("#learning_outcome" + att).attr('readonly', false);
