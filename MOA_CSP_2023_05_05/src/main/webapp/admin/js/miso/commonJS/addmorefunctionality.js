		...
		var s_t=tbl_name+prev_ser; // table name and serial 
		 ...
		 if(gc_add[tbl_no] < 51){
			 ...
			 $("table#table_"+tbl_name).append('<tr align="center" id="tr_id_'+tbl_name+gc_add[tbl_no]+'"><td>'+gc_add[tbl_no]+'</td>'
		 			...
		 			+'<td><ul class="buttons-group"><li><a class="main-btn success-btn-outline btn-hover btn-sm" value = "ADD" title = "ADD" id = "id_add_'+tbl_name+gc_add[tbl_no]+'"  ><i class="lni lni-plus"></i></a></li><li> <a class="main-btn danger-btn-outline btn-hover btn-sm" value="REMOVE" title = "REMOVE" id = "id_remove_'+tbl_name+gc_add[tbl_no]+'"><i class="lni lni-trash-can"></a></li></ul></td>' 
		   		    +'</tr>');
				...
				$("input#current_count_hidden_"+tbl_name).val(rp);		
				 $("table#table_"+tbl_name).append('<tr align="center" id="tr_id_'+tbl_name+rp+'"><td>'+rp+'</td>'
					...
					+tbody+
		 			...
		 			+'<td><a class="btn btn-success btn-sm" value = "ADD" title = "ADD" id = "id_add_'+tbl_name+rp+'"  ><i class="fa fa-plus"></i></a> <a class="btn btn-danger btn-sm" value="REMOVE" title = "REMOVE" id = "id_remove_'+tbl_name+rp+'" onclick="formopen_remove_edit('+rp+',\''+tbl_name+'\',\''+tbl_no+'\');"><i class="fa fa-trash"></a></td>' 
		   		    +'</tr>');
	 ...
	 if(gc_save[tbl_no] < 51){
		 $("table#table_"+tbl_name).append('<tr align="center" id="tr_id_'+tbl_name+gc_save[tbl_no]+'"><td>'+gc_save[tbl_no]+'</td>'
				...
				+'<td><a class="btn btn-primary btn-sm" value = "SAVE" title = "SAVE" id = "tools_save'+gc_save[tbl_no]+'" onclick="save_fn_'+tbl_name+'('+gc_save[tbl_no]+'); " ><i class="fa fa-save"></i></a>'
			 	+'<a style="display:none;" class="btn btn-success btn-sm" value = "ADD" title = "ADD" id = "id_add_'+tbl_name+gc_save[tbl_no]+'"  ><i class="fa fa-plus"></i></a>'
