...
public class E_Form_Student_Dtl_Controller {
	@Autowired
	...
	@GetMapping(value = "/e_form_nch_student_url")
	public ModelAndView e_form_nch_student_url(ModelMap model, HttpSession session,
...
//			model.put("getapp_instituteNameList", common.getapp_instituteNameList(sessionFactory));
		...
		return new ModelAndView("e_form_nch_student_Tiles");
	}

	@PostMapping(value = "/e_form_nch_student_Action")
	public @ResponseBody Map<String, String> e_form_nch_student_Action( 
			HttpServletRequest request, ModelMap model, HttpSession session, Principal principal,
			@RequestParam(value = "msg", required = false) String msg, Authentication authentication, String id, MultipartHttpServletRequest mul,
...
//		System.err.println("\n\n udid------------>>>>>>"+udid+"\n\n");
		...
		String fees_date = request.getParameter("fees_date"+indno);
		MultipartFile fees_receipt = mul.getFile("fees_receipt"+indno);
		String hid_fees_receipt = request.getParameter("hid_fees_receipt"+indno);
...
//		}
		...
		if (neet_application_no == null || neet_application_no.trim().equals("")) {
			  data.put( "err" ,  "Please Enter NEET Application No");
			  data.put( "field" ,  "neet_application_no");
			  return data;
		}
		if (neet_application_no.length() != 12) {
			  data.put( "err" ,  "Please Enter 12 digit NEET Application No");
			  data.put( "field" ,  "neet_application_no");
			  return data;
		}
		
		if (validation.isOnlyNumer(neet_application_no) == true) {
			data.put("msg", "NEET Application No " + validation.isOnlyNumerMSG);
			return data;
		}
		
		if (neet_roll_no == null || neet_roll_no.trim().equals("")) {
			  data.put( "err" ,  "Please Enter NEET Roll No");
			  data.put( "field" ,  "neet_roll_no");
			  return data;
		}
		
		if (neet_roll_no.length() != 10) {
			  data.put( "err" ,  "Please Enter 10 digit NEET Roll No");
			  data.put( "field" ,  "neet_roll_no");
			  return data;
		}
		
		if (validation.isOnlyNumer(neet_roll_no) == true) {
			data.put("err", "NEET Roll No. " + validation.isOnlyNumerMSG);
			data.put( "field" ,  "neet_roll_no");
			return data;
		}
		
		if (neet_rank == null || neet_rank.trim().equals("")) {
			  data.put( "err" ,  "Please Enter ALL INDIA NEET Rank");
			  data.put( "field" ,  "neet_rank");
			  return data;
		}
		
		if (neet_rank.length() > 7) {
			  data.put( "err" ,  "Please Enter Maximum 7 digit ALL INDIA NEET Rank");
			  data.put( "field" ,  "neet_rank");
			  return data;
		}
		if (validation.isOnlyNumer(neet_rank) == true) {
			data.put("err", "ALL INDIA NEET Rank " + validation.isOnlyNumerMSG);
			data.put( "field" ,  "neet_rank");
			return data;
		}
		
		if (neet_percentile == null || neet_percentile.trim().equals("")) {
			  data.put( "err" ,  "Please Enter NEET Percentile");
...
//		}
		...
		if (validation.isOnlyNumerandDotMSG(neet_percentile) == false) {
			data.put("err", "NEET Percentile " + validation.isOnlyNumerandDotMSG);
			data.put( "field" ,  "neet_percentile");
			return data;
		}
		...
		if (Integer.parseInt(neet_marks) > 720) {
			  data.put( "err" ,  "Please Enter Maximum 720 Marks Obtained");
			  data.put( "field" ,  "neet_marks");
			  return data;
		}		
		...
		if (fees_date == null || fees_date.trim().equals("") || fees_date.equals("DD/MM/YYYY")) {
			data.put("err", "Please Enter The Date Of Fees Payment");
			data.put( "field" ,  "fees_date");
			return data;
		}
		
		String fees_receipt_path="";
		
		if (fees_receipt.getOriginalFilename().isEmpty() && (hid_fees_receipt == null || hid_fees_receipt.equals("") )) {
			data.put("err", "Please Upload Fees Receipt");
			data.put( "field" ,  "fees_receipt");
			return data;
		}
		
		if (!fees_receipt.getOriginalFilename().isEmpty()) {
			 if (fees_receipt.getOriginalFilename().split("[.]").length > 2) {
					data.put("err", "Invalid file extension for Fees Receipt");
					data.put( "field" ,  "fees_receipt");
					return data;
			}
			
			String upload_fileEXt = FilenameUtils.getExtension(fees_receipt.getOriginalFilename()).toLowerCase();
			if (!upload_fileEXt.equals("pdf")) {
				data.put("err", "Only .pdf file extensions allowed for Fees Receipt");
				data.put( "field" ,  "fees_receipt");
				return data;
			}
			long filesize = fees_receipt.getSize() / 1024;
			if (filesize > 200) {
				
				data.put("err", "File size should be 2000 kb or lessfor Fees Receipt");
				data.put( "field" ,  "fees_receipt");
				return data;
			}
			 fees_receipt_path = common.fileupload(fees_receipt.getBytes(), fees_receipt.getOriginalFilename(),
					"fees_receipt");
		}
		
		if (fees_receipt.getOriginalFilename().isEmpty() && hid_fees_receipt != null  && !hid_fees_receipt.equals("") ) {
			fees_receipt_path =  hid_fees_receipt;
		}
		...
		try {
			...
			Date date_of_fees_pay = formatter.parse(fees_date);
			
			System.err.println("\n\n DATE OF Fees Pay --------------"+date_of_fees_pay+"\n\n");
...
//			System.err.println("university_id------------>   "+university_id);
			...
			if(hid_quali.equals("0")) {
				...
				if(role.toUpperCase().contains("NCISM")) {
					Query cneetdtlq = sessionHQL.createQuery("select count(*) from EDU_LMS_STUDENT_DETAILS where neet_application_no=:neet_application_no or neet_roll_no=:neet_roll_no or neet_rank=:neet_rank ");
					...
					Query cneetdtlq2 = sessionHQL.createQuery("select count(*) from EDU_LMS_NCH_STUDENT_DETAILS where neet_application_no=:neet_application_no or neet_roll_no=:neet_roll_no or neet_rank=:neet_rank ");
					...
					Long cneetdtlcnt2 = (Long) cneetdtlq2.uniqueResult();
					
					System.err.println("\n\n NCISM COUNT--------------------"+cneetdtlcnt+"---NCH COUNT---"+cneetdtlcnt2+"\n\n");
					
					if(cneetdtlcnt > 0 || cneetdtlcnt2 > 0) {
				...
				if(role.toUpperCase().contains("NCISM")) {
					...
					Query cscq = sessionHQL.createQuery("select count(*) from EDU_LMS_STUDENT_DETAILS where institude_userid=:inst_id and degree=:degree");
...
//					System.err.println("\n\n*************"+checkseatcount+"***"+ir.getTotal_sanctioned_seat()+"***"+name_of_institute+"\n\n");
					...
					if(checkseatcount < ir.getTotal_sanctioned_seat()) {
...
//							add_detail.setAyush_id(maxAID);
					    	...
					    	add_detail.setFees_date(date_of_fees_pay);
					    	add_detail.setFees_receipt(fees_receipt_path);
...
//					    	//////////////////////////////
				...
				if(role.toUpperCase().contains("NCH")) {
					Query cscq = sessionHQL.createQuery("select count(*) from EDU_LMS_NCH_STUDENT_DETAILS where institude_userid=:inst_id and degree=:degree");
					cscq.setParameter("inst_id",Integer.parseInt(name_of_institute));
					cscq.setParameter("degree",Integer.parseInt(degree));
					Long checkseatcount = (Long) cscq.uniqueResult();
...
//					    	System.err.println("\n\n"+add_detailNch.getInst_code()+"**********"+add_detailNch.getDegree()+"\n\n");
			    ...
			    UserLogin p = new UserLogin();
					...
					sessionHQL.flush();
					sessionHQL.clear();
					data.put("userid",String.valueOf(uid));
...
//					data.put("userid",String.valueOf(uid));
					data.put("sanc_seat",edao.E_Form_Student_DtlData(role,name_of_institute).get(0).get(5));
					data.put("admt_seat",edao.E_Form_Student_DtlData(role,name_of_institute).get(0).get(6));
					data.put("vacn_seat",edao.E_Form_Student_DtlData(role,name_of_institute).get(0).get(7));
...
//					 || cle4 > 0
				...
				if(role.toUpperCase().contains("NCISM")) {
					...
					hql = "update EDU_LMS_STUDENT_DETAILS set modified_by=:modified_by ,modified_date=:modified_date ,institude_userid=:institude_userid ,state=:state ,name=:name ,last_name=:last_name ,"
							...
							+ "university_userid=:university_userid ,email=:email,fees_date=:fees_date,fees_receipt=:fees_receipt where id=:id";
					}
					if(c1 > 0 ) {
				...
				if(role.toUpperCase().contains("NCH")) {
					...
					hql = "update EDU_LMS_NCH_STUDENT_DETAILS set modified_by=:modified_by ,modified_date=:modified_date ,institude_userid=:institude_userid ,state=:state ,name=:name ,last_name=:last_name ,"
							+ "mother_name=:mother_name ,father_name=:father_name ,dob=:dob ,neet_application_no=:neet_application_no ,neet_roll_no=:neet_roll_no ,"
							+ "neet_rank=:neet_rank ,neet_percentile=:neet_percentile ,neet_marks=:neet_marks ,quota=:quota ,counc_auth=:counc_auth ,category=:category ,"
							+ "university_userid=:university_userid ,email=:email where id=:id";
					}
					if(c2 > 0 ) {
						data.put("msg", "This "+ email +" Email is Already Registered.");
						return data;
				...
				if(role.toUpperCase().contains("NCISM")) {
					query.setParameter("fees_date", date_of_fees_pay)
					.setParameter("fees_receipt", fees_receipt_path);
				}
				...
				////////////////////
				   ...
				   msg = query.executeUpdate() > 0 ? "1" : "0";
					...
					if (role.toUpperCase().contains("NCISM")) {
						...
						Query ueilq = sessionHQL.createQuery(ueilqs).setParameter("email_id", email)
...
//					
						if (msg.equals("1") && leumsg.equals("1")) {
	...
	@RequestMapping(value = "/getInsCodeListbyInsName_ctrl", method = RequestMethod.POST)
	public @ResponseBody List<EDU_LMS_INSTITUTE_REGISTRATION> getInsCodeListbyInsName_ctrl(String name_of_institute) {
		List<EDU_LMS_INSTITUTE_REGISTRATION> list = common.getInsCodeListbyInsName(sessionFactory, name_of_institute);
		return list;
	}
	
	@RequestMapping(value = "/eformFinalsubmit", method = RequestMethod.POST)
	public @ResponseBody String eformFinalsubmit(HttpSession session,String entrycount) {
		...
		String hql = "update EDU_LMS_INSTITUTE_REGISTRATION set modified_by=:modified_by,modified_date=:modified_date,dashboard_status=:dashboard_status where id=:id";
		
		Query query = sessionHQL.createQuery(hql)
		    	.setParameter("modified_date",new Date())
		    	.setParameter("modified_by", username)
		    	.setParameter("dashboard_status",1)
		    	.setParameter("id",Integer.parseInt(institute_id));				   
...
//		   }
	...
	@RequestMapping(value = "/eformDeleteData", method = RequestMethod.POST)
	public @ResponseBody String eformDeleteData(HttpSession session,String stu_id,String email_id) {
		...
		if(roleid.equals("45") || roleid.equals("46")){
			...
			if(msg1.equals("1") && msg2.equals("1") && msg3.equals("1")) {
