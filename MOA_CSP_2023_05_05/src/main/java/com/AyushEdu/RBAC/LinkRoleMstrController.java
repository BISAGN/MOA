...
//		}
			...
			if (screen_old.equals("")) {
				for (int i = 0; i < screen_s.length; i++) {
					...
					for (int j = 0; j < screen_s1.length; j++) {
						...
						if (j == 0) {
							module1 = screen_s1[j].trim();
						} else if (j == 1) {
							submodule1 = screen_s1[j].trim();
						} else
							screen1 = screen_s1[j].trim();
					}
	...
	// For role by module in rolemaster table
		...
		public @ResponseBody List<String> getrolebymodule(int roleid, HttpSession sessionUserId) {
			...
			String nrQry = "select distinct cast(module.id as text) from TB_LDAP_ROLEMASTER where roleid=:roleid and module.id != 0  order by cast(module.id as text)";
		...
		public @ResponseBody List<String> getrolemodulebysubmod(int roleid, int moduleid, HttpSession sessionUserId) {
			Session session = this.sessionFactory.openSession();
			Transaction tx = session.beginTransaction();
			String nrQry = "select distinct cast(sub_module.id as text) from TB_LDAP_ROLEMASTER where roleid=:roleid and module.id != 0 and sub_module.id != 0 and module.id=:moduleid  order by cast(sub_module.id as text)";
			Query q = session.createQuery(nrQry);
			q.setParameter("roleid", roleid);
		...
		public @ResponseBody List<String> getrolemodulesubmodbyscreen(int moduleid, int submoduleid, int role_id,
			...
			Transaction tx = session.beginTransaction();
			String nrQry = "";
			nrQry = "select distinct cast(screen.id as text) from TB_LDAP_ROLEMASTER where roleid=:role_id and module.id != 0 and sub_module.id != 0 and module.id=:moduleid and sub_module.id =:submoduleid order by cast(screen.id as text) ";
			Query q = session.createQuery(nrQry);
