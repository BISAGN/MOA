...
@Service
@Repository
public class InstituteMasterDaoImpl_old implements InstituteMasterDao_old {
	@Autowired
...
//			} else {
	...
	public List<CoFeescategorytype> GetFees() {
		...
		try {
			Query q1 = sessionHQL.createQuery("From CoFeescategorytype where status = '1' order by feestype");
	...
	@Override
	public boolean CheckInstituteOtherDetailAlreadyExist(int insid, int feesid, int categoryid) {
		Session sessionHQL = sessionFactory.getSessionFactory().openSession();
		Transaction tx = sessionHQL.beginTransaction();
