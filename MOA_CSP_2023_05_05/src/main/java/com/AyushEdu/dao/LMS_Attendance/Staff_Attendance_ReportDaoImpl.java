...
@Repository
public class Staff_Attendance_ReportDaoImpl implements Staff_Attendance_ReportDao {
...
//					cast(exam_serial as character varying) like ?
			...
			if(!name.equals("")){
				searchvalue+=" and upper(fm.name) like '%" + name.toUpperCase() + "%'";
			}
			...
			if(role.contains("NCH")) {
			 ...
			 for (int i=1;i<=lastDay;i++) {
				...
				q = "select distinct l.userid,concat(td.first_name,' ',td.middle_name,' ',td.last_name) as name,td.teacher_code, " + temp + " from edu_lms_faculty_attendance fm\n"
						...
						+ "inner join logininformation l on l.userid=td.user_id where fm.id!=0 and rl.role=? "
						+ "and l.institute_id=(select institute_id from logininformation where userid = ?) " +searchvalue;
				...
				String temp="";
				 for (int i=1;i<=lastDay;i++) {
					...
					q = "select distinct l.userid,lfn.username,lfn.teacher_code, " + temp + " from edu_lms_faculty_attendance fm\n"
							...
							+ "inner join roleinformation rl on rl.role_id=u.role_id\n"
							+ "inner join logininformation l on l.userid=lfn.userid::int where fm.id!=0 and rl.role=? "
							+ "and l.institute_id=(select institute_id from logininformation where userid = ?) " +searchvalue;
