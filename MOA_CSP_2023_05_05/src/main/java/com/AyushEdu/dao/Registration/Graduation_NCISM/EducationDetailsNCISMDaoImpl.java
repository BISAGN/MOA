...
@Service
@Repository
public class EducationDetailsNCISMDaoImpl implements EducationDetailsNCISMDao {
	...
	public ArrayList<ArrayList<String>> DataTableEducationNcism_DataList(int startPage, int pageLength, String Search,
		...
		try {
			...
			q="select distinct ROW_NUMBER() OVER(order by ed.id) as sr_no, \n"
					+ " ed.id, am.academic, ed.passing_year,ed.institute_name,ed.obtain_marks,ed.total_marks,\n"
					+ " edm.division_name as grade,ed.doc_path, ed.board_or_university,ed.school_or_collage,ed.subject,\n"
					+ " am.id as academic_id from edu_ncism_reg_gradu_pre_edu_dtls_tbl ed\n"
...
//			q="select distinct ROW_NUMBER() OVER(order by ed.id) as sr_no, \n"
//					+" ed.id, m.label as academic, ed.passing_year,ed.institute_name,ed.obtain_marks,ed.total_marks,ed.grade,ed.doc_path, ed.board_or_university,ed.school_or_collage,ed.subject,m.codevalue as academic_id from edu_ncism_reg_gradu_pre_edu_dtls_tbl ed\n"
//					+ "inner join edu_ncism_reg_gradu_personal_dtls tpd on tpd.id = ed.p_id\n"
...
//			q = "select distinct ROW_NUMBER() OVER(order by ed.id) as sr_no, ed.id,m.label as academic,ed.passing_year,ed.institute_name,ed.obtain_marks,ed.total_marks,ed.grade,ed.doc_path from edu_ncism_reg_gradu_pre_edu_dtls_tbl ed \n"
...
//				list.add(f2);
	...
	@Override
	public long DataTableEducationNcism_DataTotalCount(String Search, String academic, Integer passing_year,
		...
		try {
			...
			q = "select count(*) from (" 
					+ "select distinct ROW_NUMBER() OVER(order by ed.id) as sr_no, \n"
					+ " ed.id, am.academic, ed.passing_year,ed.institute_name,ed.obtain_marks,ed.total_marks,\n"
					+ " edm.division_name as grade,ed.doc_path, ed.board_or_university,ed.school_or_collage,ed.subject,\n"
					+ " am.id as academic_id from edu_ncism_reg_gradu_pre_edu_dtls_tbl ed\n"
					+ "inner join edu_ncism_reg_gradu_personal_dtls tpd on tpd.id = ed.p_id\n"
...
//			}
	...
	public String getFilePathQueryNcism_(int id) {
		...
		try {
			...
			String query = null;
			query = "select doc_path from edu_ncism_reg_gradu_pre_edu_dtls_tbl where id = ?";
	...
	public String updateNcism_SubCategory(EDU_NCISM_REG_GRADU_PRE_EDU_DTLS_TBL obj){
		...
		try{
			...
			String hql = "update EDU_NCISM_REG_GRADU_PRE_EDU_DTLS_TBL set academic=:academic,passing_year=:passing_year,board_or_university=:board_or_university,school_or_collage=:school_or_collage,subject=:subject,obtain_marks=:obtain_marks,"
	...
	public ArrayList<ArrayList<String>> geteditEducation_Ncism_data(String id) {
		...
		ArrayList<ArrayList<String>> alist = new ArrayList<ArrayList<String>>();
	       ...
	       try{          
	       	conn = dataSource.getConnection();
	           String sq1=" select * from edu_ncism_reg_gradu_pre_edu_dtls_tbl WHERE id = ?";                         
	           ...
	           while(rs.next()){
	           	ArrayList<String> list = new ArrayList<String>();
					...
					list.add(rs.getString("academic"));// 1
					list.add(rs.getString("board_or_university"));// 2
					list.add(rs.getString("school_or_collage"));// 3
					list.add(rs.getString("subject"));// 4
					list.add(rs.getString("passing_year"));// 5
					list.add(rs.getString("obtain_marks"));// 6
					list.add(rs.getString("grade"));// 7
					list.add(rs.getString("doc_path"));// 8
					alist.add(list);                           	  
	           }
	           rs.close();
	           stmt.close();
	...
	public ArrayList<ArrayList<String>> Getedu_chekyear_ncism_data(String p_id) {
		ArrayList<ArrayList<String>> alist = new ArrayList<ArrayList<String>>();
	       ...
	       try{          
	       	conn = dataSource.getConnection();
	           String sq1=" select academic,passing_year from edu_ncism_reg_gradu_pre_edu_dtls_tbl WHERE p_id =?";                         
