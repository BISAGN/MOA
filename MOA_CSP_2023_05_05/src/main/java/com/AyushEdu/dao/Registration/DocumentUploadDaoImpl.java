...
q = " select distinct ROW_NUMBER() OVER(order by d.id) as sr_no, d.id,dam.doc_name,d.upload_document \n"
					...
					+ "inner join edu_reg_gradu_personal_dtls tpd on tpd.id = d.p_id\n"
...
//				list.add(rs.getString("upload_document"));//2
				...
				list.add(rs.getString("upload_document"));//1
				i++;
					alist.add(list);
			}
	...
	public long DataTableDocDataTotalCount(String Search, Integer doc_name,int userid) {
		...
		try {
			...
			//q = "select count(*) \n" + "from edu_reg_gradu_other_doc_upload d where id!=0 " + SearchValue;
			PreparedStatement stmt = conn.prepareStatement(q);
	...
	public String GenerateQueryWhereClause_SQL(String Search, Integer doc_name,int userid) {
		...
		if (Search != null && !Search.equals("")) { // for Input Filter
			SearchValue += "and upper(dam.doc_name) like ? ";
...
//			}
	...
	public String getFilePathQuery(int id) {
		...
		try {
			...
			String query = null;
			query = "select upload_document from edu_reg_gradu_other_doc_upload where id = ?";
	...
	public String getSignatureImagePath(String p_id) {
		...
		try {
			Query q1 = sessionHQL
					.createQuery("SELECT signature FROM TB_DOCUMENT_UPLOAD where p_id=:p_id order by id DESC");
	...
	public String getPhotographImagePath(String p_id) {
		...
		try {
			Query q1 = sessionHQL
					.createQuery("SELECT photograph FROM TB_DOCUMENT_UPLOAD where p_id=:p_id order by id DESC");
			q1.setParameter("p_id", Integer.parseInt(p_id));
			@SuppressWarnings("unchecked")
			List<String> list = (List<String>) q1.list();
			String path=list.get(0);
	...
	public ArrayList<ArrayList<String>> get_uploded_imgthumb_data(String p_id) {
       ...
       try{          
       	conn = dataSource.getConnection();
           String sq1=" select id,photograph,signature,court_order from edu_reg_gradu_document_upload  WHERE p_id = ?";                         
       ...
       System.err.println("get_uploded_imgthumb_data--------->   "+alist);
