...
@Repository
public class Std_Pers_Details_Daoimpl implements Std_Pers_Details_Dao {
	...
	public TB_PERSONAL_DETAILS getStudDetViewByid(int id) {
		...
		TB_PERSONAL_DETAILS viewid = (TB_PERSONAL_DETAILS) sessionHQL.get(TB_PERSONAL_DETAILS.class, id);
	...
	public ArrayList<ArrayList<String>> getEdu_Detail_data(String p_id) {
		...
		try {
			...
			q="select distinct ROW_NUMBER() OVER(order by ed.id) as sr_no, \n"
					+" ed.id, am.academic, ed.passing_year,ed.institute_name,ed.obtain_marks,ed.total_marks,edm.division_name as grade,ed.doc_path, ed.board_or_university,ed.school_or_collage,ed.subject,ed.id as docid from edu_reg_gradu_pre_edu_dtls_tbl ed\n"
	...
	public ArrayList<ArrayList<String>> get_ayush_idbypid_data(String pid) {
	    ...
	    try{          
	        ...
	        String sq1=" select \n"
	        		...
	        		+" inner join edu_reg_gradu_personal_dtls pd on pd.p_id= l.userid\n"
	       ...
	       System.err.println("stmt-----15feb23-----"+stmt);
	        ResultSet rs = stmt.executeQuery();  
	        ...
	        System.err.println("alist---6 feb-------"+alist);
	...
	public ArrayList<ArrayList<String>> getuploaded_doc_NCH_data(String p_id) {
		...
		try {
			...
			q="select distinct ROW_NUMBER() OVER(order by d.id) as sr_no, d.id,dam.doc_name,d.id as docid\n"
					+ "	from edu_reg_gradu_other_doc_upload d \n"
			...
			PreparedStatement stmt = conn.prepareStatement(q);
			stmt.setInt(1, Integer.parseInt(p_id));
			System.err.println("document----->"+stmt);
			ResultSet rs = stmt.executeQuery();
	...
	String enddate = "";
	 ...
	 //end
	 	try {
	        ...
	        String sq1=" select sd.id,pd.p_id as userid,pd.pers_aadhar_no from edu_lms_nch_student_details sd \n"
	        		+ "inner join  edu_reg_gradu_personal_dtls pd on pd.pers_email = sd.email\n"
	        		+ "WHERE pd.id = ? order by id desc";                    
	      ...
	      String maxAID = addao.getAyushID(list.get(1),session);
					...
					Transaction  tx= sessionHQL.beginTransaction();
					EDU_LMS_NCH_STUDENT_DETAILS sd = (EDU_LMS_NCH_STUDENT_DETAILS) sessionHQL .get(EDU_LMS_NCH_STUDENT_DETAILS.class, id);
					...
					if (sd.getAyush_id() == null || sd.getAyush_id().equals("null") || sd.getAyush_id().equals("")) {
						...
						Boolean Directory = directory.SaveAyushId_Directory(maxAID, String.valueOf(list.get(1)), list.get(2), "45", session);
					}
	...
	public ArrayList<ArrayList<String>> getuploaded_Court_Order_NCH_data(String p_id) {
		...
		try {
			...
			q="select doc.id,doc.court_order,p_id,remarks from edu_reg_gradu_document_upload doc  where doc.id!=0  and court_order is not null and doc.p_id = ?";
			...
			while (rs.next()) {
				...
				String vd = "<ul class='mainbtn action daobtn'><li><a class='main-btn dark-btn btn-hover btn-iconic-icon view_uploadDraftdoc'  value='ADD' title='View Data' >\n"
							+ "		<i class='lni lni-eye'>"
