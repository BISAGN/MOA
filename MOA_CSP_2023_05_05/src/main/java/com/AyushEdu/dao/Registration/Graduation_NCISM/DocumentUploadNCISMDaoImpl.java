...
@Service
@Repository
public class DocumentUploadNCISMDaoImpl implements DocumentUploadNCISMDao{
	...
	public ArrayList<ArrayList<String>> DataTableDocNcism_DataList(int startPage, int pageLength, String Search,
		...
		try {
			...
			if (pageLength == -1) {
				...
				q = " select distinct ROW_NUMBER() OVER(order by d.id) as sr_no, d.id,dam.doc_name,d.upload_document \n"
					+ "from edu_ncism_reg_gradu_other_doc_upload d \n"
					+ "inner join edu_ncism_reg_gradu_personal_dtls tpd on tpd.id = d.p_id\n"
...
//				list.add(rs.getString("upload_document"));//2
				...
				list.add(rs.getString("upload_document"));
	...
	public long DataTableDocNcism_DataTotalCount(String Search, Integer doc_name,int userid) {
		...
		try {
			...
			q = "select count(*) from (\n"
					+" select distinct ROW_NUMBER() OVER(order by d.id) as sr_no, d.id,dam.doc_name,d.upload_document \n"
					+ "from edu_ncism_reg_gradu_other_doc_upload d \n"
					+ "inner join edu_ncism_reg_gradu_personal_dtls tpd on tpd.id = d.p_id\n"
					+ "inner join logininformation mp2 on  mp2.userid =  tpd.p_id\n"
					+ "inner join edu_doc_attachments_mstr dam on  dam.id =  d.doc_name\n"
			...
			//q = "select count(*) \n" + "from edu_ncism_reg_gradu_other_doc_upload d where id!=0 " + SearchValue;
			PreparedStatement stmt = conn.prepareStatement(q);
	...
	public String GenerateQueryWhereClause_SQL(String Search, Integer doc_name,int userid) {
		...
		if (Search != null && !Search.equals("")) { // for Input Filter
			SearchValue += "and upper(m.label::text) like ? ";
...
//			}
	...
	public String getFilePathNcism_Query(int id) {
		...
		try {
			...
			String query = null;
			query = "select upload_document from edu_ncism_reg_gradu_other_doc_upload where id = ?";
	...
	public String getSignatureImagePath_Ncisam(String p_id) {
		...
		try {
			Query q1 = sessionHQL
					.createQuery("SELECT signature FROM EDU_NCISM_REG_GRADU_DOCUMENT_UPLOAD where p_id=:p_id order by id DESC");
	...
	public String getPhotographImagePath_Ncisam(String p_id) {
		...
		try {
			Query q1 = sessionHQL
					.createQuery("SELECT photograph FROM EDU_NCISM_REG_GRADU_DOCUMENT_UPLOAD where p_id=:p_id order by id DESC");
			q1.setParameter("p_id", Integer.parseInt(p_id));
			@SuppressWarnings("unchecked")
			List<String> list = (List<String>) q1.list();
			String path=list.get(0);
	...
	public ArrayList<ArrayList<String>> get_uploded_imgthumb_Ncism_data(String p_id) {
       ...
       try{          
       	conn = dataSource.getConnection();
           String sq1=" select id,photograph,signature,court_order from edu_ncism_reg_gradu_document_upload  WHERE p_id = ?";                         
           ...
           while(rs.next()){
           	ArrayList<String> list = new ArrayList<String>();
				...
				list.add(rs.getString("court_order"));// 3
				alist.add(list);                           	  
           }
           rs.close();
           stmt.close();
