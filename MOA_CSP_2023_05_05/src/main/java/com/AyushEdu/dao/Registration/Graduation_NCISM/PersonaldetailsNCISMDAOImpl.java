...
@Repository
public class PersonaldetailsNCISMDAOImpl implements PersonaldetailsNCISMDAO {
...
//	}
	 ...
	 public ArrayList<ArrayList<String>> getAllNcism_Persdetails(int userid) {
	        ...
	        try{          
	        	conn = dataSource.getConnection();
	            String sq1="select gm.gender_name as gender_name,cm.category as category,rm.religion as religion,mm.marital_status as marital_status,nm.name as nationality,sm.state_name as state_name,dm.district_name as district_name,* from edu_ncism_reg_gradu_personal_dtls pd "
	            ...
	            ResultSet rs = stmt.executeQuery();  
	            System.err.println("stmtpp---"+stmt);
	            String str1="";
	            ...
	            while(rs.next()){
					...
					list.add(rs.getString("gender_name"));//56
					list.add(rs.getString("category"));//57
					list.add(rs.getString("religion"));//58
					list.add(rs.getString("marital_status"));//59
					list.add(rs.getString("nationality"));//60
					list.add(rs.getString("state_name"));//61
					list.add(rs.getString("district_name"));//62
...
public ArrayList<ArrayList<String>> getBesicNcism_details(int userid) {
	        ...
	        try{          
	            ...
	            String sq1="select l.mobile_no,l.email_id,l.login_name,l.aadhar_no,\n"
	            		+ "sd.neet_roll_no,sd.neet_application_no,sd.neet_marks,sd.neet_percentile,sd.neet_rank,	"
	            		+ "TO_CHAR(sd.dob,'DD/MM/YYYY') as dob,sd.father_name,sd.mother_name,sd.last_name,sd.quota,sd.counc_auth,sd.category\n"
	            		+ "	   from logininformation l\n"
	            		+ "	 inner join edu_lms_student_details sd on lower(sd.email)=lower(l.email_id)\n"
	            		+ "	  where userid = ?"; 
	            
	            PreparedStatement stmt = conn.prepareStatement(sq1);
	            stmt.setInt(1, userid);
	            System.err.println("------>>>besicncism-----"+stmt);
...
public ArrayList<ArrayList<String>> get_p_id_Ncism_pers_info_data(int userid) {
       ...
       try{          
       	conn = dataSource.getConnection();
           String sq1=" select id from edu_ncism_reg_gradu_personal_dtls WHERE p_id = ?";                         
...
//	        System.err.println("hiiiiiiiiiiii -->   "+  stmt);
