...
import com.AyushEdu.Models.helpdesk.HD_MARQUEE;
...
//	{
//		String hql ="";
//		hql ="from TB_LDAP_MODULE_MASTER b where b.id > 0 and b.id in (select a.module.id from TB_LDAP_ROLEMASTER a where a.roleid=:roleid and  a.module.id != 0 and b.type=1) order by b.id";
//		Session session= this.sessionFactory.openSession();
//		Transaction tx = session.beginTransaction();
//		Query q = session.createQuery(hql);
...
//	}
	...
	public List<TB_LDAP_MODULE_MASTER> getModulelist(String roleid)
	{
		...
		String hql ="";
		hql ="from TB_LDAP_MODULE_MASTER b where b.id > 0 and b.id in (select a.module.id from TB_LDAP_ROLEMASTER a where a.roleid  in $roleid and  a.module.id != 0 ) order by b.id";
		Session session= this.sessionFactory.openSession();
...
//		q.setParameter("roleid", roleid);
	...
	public List<TB_LDAP_SUBMODULE_MASTER> getSubModulelist(int  moduleid,String roleid)
	{
		...
		String hql ="from TB_LDAP_SUBMODULE_MASTER b where b.module.id =:moduleid and b.id in (select a.sub_module.id from TB_LDAP_ROLEMASTER a where a.roleid in $roleid) order by b.id ";
		Session session= this.sessionFactory.openSession();
...
//		q.setParameter("roleid", roleid);
	...
	public List<TB_LDAP_SCREEN_MASTER> getScreenlist(int  moduleid,int submoduleid,String roleid)
	{
		...
		String hql ="from TB_LDAP_SCREEN_MASTER b where b.id in (select a.screen.id from TB_LDAP_ROLEMASTER a where a.roleid in $roleid and  a.module.id =:moduleid and a.sub_module.id =:submoduleid) order by b.id";
		Session session= this.sessionFactory.openSession();
...
//		 	            }
	...
	public Boolean getdataformadatory(int user_id) {
		...
		Query q  = session.createQuery("select id FROM REG_NCH_REGISTRATION_A  where user_id =:user_id  ");
		 q.setParameter("user_id", user_id );
		@SuppressWarnings("unchecked")
		List<String> list = (List<String>) q.list();
		 
		 System.err.println("q---"+q);
		 System.err.println("iiiii---"+list);
	...
	public Boolean isaadhaarverified(Integer uId) {
		String hql="select aadhaar_is_verified from UserLogin where userId=:uId ";
		
		System.err.println("uId"+uId);
		
		String msg="";
		
		List<Boolean> users = null;
		Session session = this.sessionFactory.openSession();
		Transaction tx = session.beginTransaction();
		try {
			Query query= session.createQuery(hql);
			...
			System.out.println();
			users = (List<Boolean>) query.list();
			
			  Boolean.valueOf(users.get(0));
			
			System.err.println("adhar status-----daoimpl---->   "+users.get(0).toString());
			
//			 boolean hi = users.get(0) != null;
//			System.err.println("adhar status-----daoimpl---->       "+ hi);
...
//		}
	...
	public List<Role> findRole_url2(String role) {
		String q1="";
		String hql="FROM Role U where U.role in $role";
		Session session= this.sessionFactory.openSession();
		q1="(";
		...
		q1+=")";
		hql=hql.replace("$role",q1);
		
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery(hql);
//		q.setParameter("role", "Student_NCH");
//		q.setParameter("role1", "Faculty_NCH");
		List<Role> list = (List<Role>) q.list();
		tx.commit();
		session.close();
		if(list.size()>0)
